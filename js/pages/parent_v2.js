...
// ── ライン登録（Phase2 自動採番） ───────────────────────────
Pages.lineNew = function () {
  const main    = document.getElementById('main');
  const parents = Store.getDB('parents') || [];
  const males   = parents.filter(p => p.sex === '♂' && p.status === 'active');
  const females = parents.filter(p => p.sex === '♀' && p.status === 'active');
  const year    = new Date().getFullYear();
  const today   = new Date().toISOString().split('T')[0];

  main.innerHTML = `
    ${UI.header('ライン登録', { back: true })}
    <div class="page-body">

      <div class="form-section">
        <label class="form-label">♂ 種雄</label>
        <select id="inp-father" class="form-input" onchange="_linePreviewCode()">
          <option value="">選択してください</option>
          ${males.map(m =>
            `<option value="${m.par_id}">${m.parent_display_id || m.display_name} (${m.size_mm || '?'}mm)</option>`
          ).join('')}
        </select>
      </div>
      ...
      <div class="form-section">
        <label class="form-label">♀ 種雌</label>
        <select id="inp-mother" class="form-input">
          <option value="">選択してください</option>
          ${females.map(f =>
            `<option value="${f.par_id}">${f.parent_display_id || f.display_name} (${f.size_mm || '?'}mm)</option>`
          ).join('')}
        </select>
      </div>

      <div class="form-section">
        <label class="form-label">ラインコード（自動生成）</label>
        <div id="line-code-preview" class="auto-id-preview">♂を選択すると自動表示</div>
      </div>

      <div class="form-section">
        <label class="form-label">孵化年</label>
        <input id="inp-year" class="form-input" type="number" value="${year}">
      </div>

      <div class="form-section">
        <label class="form-label">ペアリング日</label>
        <input id="inp-pairing-date" class="form-input" type="date" value="${today}">
      </div>

      <div class="form-section">
        <label class="form-label">産地</label>
        <input id="inp-locality" class="form-input" type="text" value="Guadeloupe">
      </div>

      <button class="btn btn-primary btn-full" style="margin-top:24px"
              onclick="_lineSaveV2()">💾 ラインを登録</button>
    </div>`;
};

function _linePreviewCode() {
  const fatherId = document.getElementById('inp-father').value;
  const parents  = Store.getDB('parents') || [];
  const father   = parents.find(p => p.par_id === fatherId);
  const el       = document.getElementById('line-code-preview');
  if (!father || !el) return;

  const pid = father.parent_display_id || '';
  const m   = pid.match(/^M\d{2}-([A-Z]+)/);
  const maleCode = m ? m[1] : '?';
  el.textContent = `${maleCode}N（Nはこの♂の何本目かのライン番号 / 登録時に自動決定）`;
}

async function _lineSaveV2() {
  const fatherId = document.getElementById('inp-father').value;
  const motherId = document.getElementById('inp-mother').value;
  const year     = document.getElementById('inp-year').value;
  if (!fatherId || !motherId) { UI.toast('♂と♀を選択してください'); return; }

  try {
    UI.loading(true);
    const res = await API.phase2.createLine({
      father_par_id:  fatherId,
      mother_par_id:  motherId,
      hatch_year:     year,
      pairing_date:   document.getElementById('inp-pairing-date').value,
      locality:       document.getElementById('inp-locality').value,
    });
    await Store.refreshDB();
    UI.toast(`ライン ${res.display_id} を登録しました`);
    routeTo('line-list');
  } catch(e) {
    UI.toast('エラー: ' + e.message, 'error');
  } finally {
...
function _clientExtractTags2(raw) {
